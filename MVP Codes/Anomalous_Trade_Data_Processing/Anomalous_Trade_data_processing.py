'''
This is the code for data flow(Stream) in persist/tranform layer of architecture diagram.
It reads data from the stream pool, appends three attributes which are start time and end time of 30 second interval and market value.
Then appends this data into the .csv file which is stored in Silver bucket (ratd_silver_bucket).
'''
from pyspark.sql import SparkSession
from pyspark.sql.functions import concat, col, window, from_json
import argparse
import os
...
def main():
  ...
  # Display necessary Message if Exception occurs.
 ...
 # Credentials of kafka stream
  ...
  # Add start time and end time of 30 seconds interval to the trade dataframe.
  washTrades_df = (
      tradedata_df3
...
# Write data to CSV file in Silver Data lake
  ...
  # To continously run the Data flow 24/7
